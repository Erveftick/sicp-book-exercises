...
(comment
 "Небольшой отзыв: (можно пропустить, решения на 60 строке)
 по началу книга шла очень тяжело, всё через силу, много
 математики и теоретики, но ближе к задаче 1.20 стало хоть
 чуть-чуть интересно и менее сложно, не смотря на то, что дальше
 были числа Фиббоначи и прочие формулы, которые казались страшными.
 Возможно, задачи как были сложными, так и остались, но я стал
 тратить на них меньше времени, чем приходилось тратить на задачи
 1.1 - 1.20 (+-).

 Все задачи связаны друг с другом, и я читал все разделы, прежде чем
 приступить к заданиям, но даже после прочтения, когда в задании
 ссылались на то, что `используйте функцию Х из пункта Y`, то иногда
 приходилось перечитывать некоторые вещи, более детально смотреть на
 представленные функции.

 Математика представленная в книге хоть и выглядит страшной, таковой
 не является. Возможно у меня сыграл опыт `прикладного математика`,
 но я уверен, что обычному человеку эти задачи по силам, хоть и с
 бОльшим затраченным временем

 Статистика(на коленке):
 Время:
 - Я не трекал время (да и думаю, что не буду в будущем),
 затраченное на каждое из заданий, но некоторые поддавались
 спустя 3 минуты, на некоторые у меня уходило >2 часов(вначале).
 Но по ощущениям, учитывая все задачи, я бы сказал, что в среднем
 на задачу 35-45 минут.
 - В целом книгу начал 2021-11-25, решил, может быть, 6 задач,
 по понятным причинам забил, потом 2022-08-13 вернулся и
 закончил 2022-09-02. В сроки не укладываюсь;)

 Задачи:
 - Любимые задачи это задания, где тупо формулу вбиваешь в
 функцию, и не думаешь. Таких было 5-7 заданий. На них
 я потратил в сумме не более 2 часов, я думаю.

 - Мои самые нелюбимые задачи были на 'покажите что алгоритм по
 памяти и времени O(log n)'. Очень трудоемкие задания: сначала
 придумать алгоритм, потом подумать, дает он log скорость или
 нет, еще и память... это всегда время и силы.")
...
(comment
 ...
 (/ 6 2) ; => 3
 (+ (* 2 4) (- 4 6)) ; => 6
 (def a 3) ; => #'sicp-book-exercises.chapter1/a
 (def b (+ a 1)) ; => #'sicp-book-exercises.chapter1/b
 (+ a b (* a b)) ; => 19
 (= a b) ; => false
...
(comment
 ((double-fn inc) 1) ; => 3
 (((double-fn (double-fn double-fn)) inc) 5) ; => 21
 )

; 1.42

(defn compose [fn-1 fn-2]
  (fn [x] (fn-1 (fn-2 x))))

(comment
 ; Мой вариант решения
 ((compose square inc) 6) ; => 49
 ; Для проверки уже реализованная функция из Clojure
...
(defn repeated [f n]
  (fn [x]
    (loop [counter n
           num     x]
      (if (> counter 0)
        (recur (dec counter) (f num))
        num))))

(comment
 ((repeated square 3) 5)
 ; Подсказку не совсем понял как использовать, сделал как понял
 )
...
(defn smooth [f]
  (fn [x]
    (let [major-x   (f (+ x dx))
          default-x (f x)
          minor-x   (f (- x dx))]
      (/ (+ minor-x default-x major-x) 3))))

(defn smooth-n-times [f n]
  ((repeated smooth n) f))

(comment
 (smooth-n-times inc 5) ; => Ответом является сглаженная функция
 )
...
(defn task-1-45 [x n damp]
  (fixed-point ((repeated average-damp damp) #(/ x (Math/pow % (dec n)))) 1.0))

(comment
 (task-1-45 2 1 1) ; => 1.9999923706054688
 (task-1-45 2 2 1) ; => 1.4142135623746899
 (task-1-45 2 3 1) ; => 1.259923236422975
 (task-1-45 2 4 1) ; => StackOverflowError, то есть, нужно повысить кол-во усреднений
 (task-1-45 2 4 2) ; => 1.189207115002721
 ; Для чисел 4 - 7 достаточно 2 усреднений, чтобы начала работать программа
 (task-1-45 2 8 2) ; => StackOverflowError, то есть, нужно повысить кол-во усреднений
 (task-1-45 2 8 3) ; => 1.090507732665258
 ; Для чисел 8 - 15 достаточно 3 усреднений, чтобы начала работать программа
 (task-1-45 2 16 3) ; => StackOverflowError, то есть, нужно повысить кол-во усреднений
 (task-1-45 2 16 4) ; => 1.0442737824274144
 ; На этом моменте я начал замечать закономерность между степенью y (числа n) и
 ; количества усреднений: 4-2, 8-3, 16-4, предположил что для 32 нужно 5 и оказался прав
 (task-1-45 2 32 4) ; => StackOverflowError
 (task-1-45 2 32 5) ; => 1.0218971486541175
 ; Выходит, что число n должно не превосходить 2^damp
 ; Тогда можно вообще от числа damp избавиться, если преобразуем функцию
 ; как логарифм n по 2 (с округлением вниз)
 )

(defn log-base-2 [n]
  (/ (Math/log n) (Math/log 2)))

(defn better-task-1-45 [x n]
  (let [new-damp (log-base-2 n)]
    (fixed-point ((repeated average-damp new-damp) #(/ x (Math/pow % (dec n)))) 1.0)))

(comment
 (better-task-1-45 2 32) ; => 1.0218971486541175
 ; Всё верно
 )
...
(defn iterative-improve [good-enough-fn? improve-fn]
  (defn iterative-improve-iter [guess]
    (let [next (improve-fn guess)]
      (if (good-enough-fn? guess next)
        next
        (iterative-improve-iter next))))
  (fn [guess] (iterative-improve-iter guess)))

(defn sqrt-1-46 [x]
  (defn improve-1-46 [guess]
    (average guess (/ x guess)))
  (defn good-enough-1-46? [next guess]
    (< (abs (- (square next) x)) 0.001))
  ((iterative-improve good-enough-1-46? improve-1-46) 1.0))

(comment
 (sqrt-1-46 2) ; => 1.4142135623746899
 (sqrt-1-46 4) ; => 2.000000000000002
 (sqrt-1-46 16) ; => 4.000000000000051
 )

(defn fixed-point-1-46 [f first-guess]
  (defn close-enough-1-46? [v1 v2]
    (< (abs (- v1 v2)) tolerance))
  (defn improve-guess-1-46 [guess]
    (f guess))
  (defn try-1-46 [guess]
    (let [next (improve-guess-1-46 guess)]
      (if (close-enough-1-46? guess next)
        next
        (try-1-46 next))))
  (try-1-46 first-guess))

(comment
 (fixed-point #(+ 1 (/ 1 %)) 1.0) ; => 1.6180327868852458
 (fixed-point-1-46 #(+ 1 (/ 1 %)) 1.0) ; => 1.6180327868852458
 )


(comment
 "На этом первый раздел книги SICP закончен.

 Небольшой отзыв: по началу книга шла очень тяжело, всё через
 силу, много математики, но ближе к задаче 1.20 стало хоть
 чуть-чуть интересно и менее сложно, не смотря на то, что дальше
 были числа Фиббоначи и прочие формулы, которые казались страшными.
 Возможно, задачи как были сложными, так и остались, но я стал
 тратить на них меньше времени, чем приходилось тратить на задачи
 ...
 спустя 3 минуты, на некоторые у меня уходило >2 часов(вначале).
 Но по ощущениям, учитывая все задачи, я бы сказал, что в среднем
 на задачу 25-45 минут. Можно и быстрее, конечно, но `втянутся`
 дело не быстрое.
 - В целом книгу начал 2021-11-25, решил, может быть, 6 задач,
 по понятным причинам забил, потом 2022-08-13 вернулся и
 закончил 2022-09-02. В сроки не укладываюсь;)
